...
QList<QgsMapLayerServerProperties::WmsDimensionInfo> QgsServerApiUtils::temporalDimensions( const QgsVectorLayer *layer )
{
  ...
  dimensions.erase(
    std::remove_if( dimensions.begin(), dimensions.end(), []( QgsMapLayerServerProperties::WmsDimensionInfo &dim ) { return dim.name.toLower() != u"time"_s && dim.name.toLower() != "date"_L1; } ),
    dimensions.end()
  );
