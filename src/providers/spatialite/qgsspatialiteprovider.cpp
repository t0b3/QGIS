...
bool QgsSpatiaLiteProvider::getTableSummary()
{
  ...
  if ( !mGeometryColumn.isEmpty() )
  {
    sql += QStringLiteral( ", Min(MbrMinX(%1)), Min(MbrMinY(%1)), Min(ST_MinZ(%1)), Max(MbrMaxX(%1)), Max(MbrMaxY(%1)), Max(ST_MaxZ(%1))" ).arg( QgsSqliteUtils::quotedIdentifier( mGeometryColumn ) );
  }
