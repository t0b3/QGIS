...
void TestQgsMapToolAddRing::testAddRing()
{
  ...
  std::unique_ptr<QgsMapMouseEvent> event( new QgsMapMouseEvent( mCanvas, QEvent::MouseButtonRelease, mapToPoint( 1, 1 ), Qt::LeftButton ) );
...
void TestQgsMapToolAddRing::testAddRingClockWise()
{
  ...
  // Draw in clockwise
  std::unique_ptr<QgsMapMouseEvent> event( new QgsMapMouseEvent( mCanvas, QEvent::MouseButtonRelease, mapToPoint( 3, 3 ), Qt::LeftButton ) );
  mCaptureTool->cadCanvasReleaseEvent( event.get() );
